...
def channel_attention(input_xs, reduction_ratio=0.125):
    ...
    Dense_One = KL.Dense(units=int(channel * reduction_ratio), activation='relu', kernel_initializer='he_normal', use_bias=True, bias_initializer='zeros',name=str(uuid.uuid4()))
    Dense_Two = KL.Dense(units=int(channel), activation='relu', kernel_initializer='he_normal', use_bias=True, bias_initializer='zeros',name=str(uuid.uuid4()))
    # max path
...
def spatial_attention(channel_refined_feature):
    ...
    return KL.Conv2D(filters=1, kernel_size=(3, 3), padding="same", activation='sigmoid', kernel_initializer='he_normal', use_bias=False,name=str(uuid.uuid4()))(max_avg_pool_spatial)
